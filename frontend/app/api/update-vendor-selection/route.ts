...
export async function POST(request: NextRequest) {
  try {
    const { requestId, vendorName, isSelected } = await request.json();

    if (!requestId || !vendorName || typeof isSelected !== 'boolean') {
      return NextResponse.json(
        { success: false, error: 'Request ID, Vendor Name, and isSelected (boolean) are required' },
        { status: 400 }
      );
    }
    ...
    console.log('Updating vendor selection:', { requestId, vendorName, isSelected });
    ...
    if (response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: result.error || 'Failed to update vendor selection' },

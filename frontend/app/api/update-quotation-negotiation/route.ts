...
export async function POST(request: NextRequest) {
  try {
    const { requestId, vendorName, negotiationNotes, negotiatedAmount } = await request.json();

    if (!requestId || !vendorName) {
      return NextResponse.json(
        { success: false, error: 'Request ID and Vendor Name are required' },
        { status: 400 }
      );
    }
    ...
    console.log('Updating quotation negotiation data:', { requestId, vendorName, negotiationNotes, negotiatedAmount });
    ...
    if (response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: result.error || 'Failed to update quotation negotiation data' },
        { status: response.status }
      );
    }
  } catch (error) {
    console.error('Error updating quotation negotiation data:', error);
    return NextResponse.json(

...
export async function POST(request: NextRequest) {
  try {
    const { requestId, vendorName, isAccepted } = await request.json();

    if (!requestId || !vendorName || typeof isAccepted !== 'boolean') {
      return NextResponse.json(
        { success: false, error: 'Request ID, Vendor Name, and isAccepted (boolean) are required' },
        { status: 400 }
      );
    }
    ...
    console.log('Updating agreement acceptance:', { requestId, vendorName, isAccepted });
    ...
    if (response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: result.error || 'Failed to update agreement acceptance' },
        { status: response.status }
      );
    }
  } catch (error) {
    console.error('Error updating agreement acceptance:', error);
    return NextResponse.json(
